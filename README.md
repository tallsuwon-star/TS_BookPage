...
- 반대로, **"데이터 초기화" 버튼을 누르기 전까지는** 업로드/처리한 내용이
  ...
  항상 그대로 남아있습니다.
- ⚠️ **(2026-09-10에 고침) 데이터를 읽어올 때 `raw.githubusercontent.com`을
  쓰지 않습니다.** 이 주소는 뒤에 CDN이 있어서, 저장 직후 새로고침(심지어
  강력 새로고침)해도 방금 커밋한 최신 내용이 아니라 캐시된 예전 내용이
  내려오는 경우가 있었습니다 — 실제로 "업로드했는데 새로고침하니
  사라졌다"는 문제의 원인이었습니다. 저장할 때 쓰는 것과 같은 GitHub
  Contents API(`api.github.com`)로 읽기도 통일해서, 캐싱 지연 없이 항상
  최신 커밋 내용을 가져오도록 고쳤습니다(`githubStorage.js`의
  `fetchRemoteData`/`getFileContent`).
