...
3. **실제 회원 개인정보(이름, 연락처, 이메일 등)는 이 저장소(`data.json`)에
   ...
   PII 열을 아예 추출하지 않습니다. 다만 "교재 주문/재고관리"(주문/
   `ORDER_EXTRA_FIELD_DEFS`)는 팀에서 **항상 사전에 가명·임의 연락처로
   치환한 파일만 업로드한다는 전제**로 주문자명/이메일/연락처/주문번호를
   마스킹 없이 저장합니다. **이 전제가 깨지면(실제 개인정보 파일을 올리게
   되면) 즉시 이 필드들을 다시 마스킹하거나 제거해야 합니다** — 그 전까지는
   업로드 버튼을 누를 때마다 뜨는 확인 창("이 페이지는 보호되지 않아...")이
   마지막 안전장치입니다. 회원 개인정보를 본격적으로 다뤄야 하는 시점이
   오면, 이 방식(GitHub 저장소를 DB처럼 쓰는 방식)을 계속 쓰면 안 되고
